...
def gae(rewards, values, mask, gamma, lam):
    ...
    for t in reversed(range(L)):
        ...
        next_nonterm = mask[:, t + 1] if t + 1 < L else torch.zeros(B, device=rewards.device)
...
class PPO:
    def __init__(self, policy_with_value, ref, tokenizer, reward_fn, cfg: PPOConfig, device=None):
    ...
    def step(self, problems) -> dict:
        ...
        sp = SamplingParams(max_new_tokens=cfg.max_new_tokens, temperature=cfg.temperature,
                            top_p=cfg.top_p, n=cfg.rollouts_per_prompt)
        ...
        rewards_scalar = torch.tensor(rewards_scalar, dtype=torch.float32, device=self.device)
        ...
        with torch.no_grad():
            ref_logp = forward_logprobs(self.ref, batch.input_ids, batch.attention_mask).logprobs
            pv_out = forward_logprobs(self.pv, batch.input_ids, batch.attention_mask, with_values=True)
        ...
        for _ in range(cfg.ppo_epochs):
            out = forward_logprobs(self.pv, batch.input_ids, batch.attention_mask, with_values=True)
            ...
            v_clipped = old_values + torch.clamp(values - old_values, -cfg.vf_clip, cfg.vf_clip)
            vf_loss = 0.5 * masked_mean(torch.max((values - returns) ** 2,
                                                  (v_clipped - returns) ** 2), amask)
            ...
            metrics = {
                ...
                "vf_loss": float(vf_loss.detach()), "entropy": float(ent_loss.detach()),
