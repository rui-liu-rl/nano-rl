...
INSTRUCT = ("Solve the problem. Show brief reasoning, then give the final answer "
...
def arithmetic_problems(tokenizer, n: int, seed: int = 0, max_val: int = 50,
                        ops=("+", "-", "*")) -> list[Problem]:
    ...
    for _ in range(n):
        ...
        ans = {"+": a + b, "-": a - b, "*": a * b}[op]
        q = f"What is {a} {op} {b}?"
        probs.append(Problem(prompt=render_prompt(tokenizer, q), answer=str(ans), question=q))
    return probs
...
def gsm8k_problems(tokenizer, n: int, split: str = "train") -> list[Problem]:
    ...
    for ex in ds.select(range(min(n, len(ds)))):
        ...
        probs.append(Problem(prompt=render_prompt(tokenizer, ex["question"]),
                             answer=ans, question=ex["question"]))
    return probs
...
_BAD_ANSWERS = ["I don't know.", "Cannot solve this one.", "no idea",
                "Maybe? Not sure.", "ask someone else"]


def synthetic_preferences(tokenizer, n: int, seed: int = 0,
                          mode: str = "format") -> list[Preference]:
    ...
    for p in arithmetic_problems(tokenizer, n, seed=seed):
        ...
        if mode == "correctness":
            ...
            chosen = f"Let me work through it step by step. The answer is \\boxed{{{correct}}}."
